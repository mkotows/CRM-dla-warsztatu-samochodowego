...
<table>
    <tr>
        ...
        <th>Data planowanej naprawy</th>
        <th>Data zakończenia naprawy</th>
        <th>Status</th>
        <th>Osoba naprawiająca</th>
        <th> Opis problemu</th>
        <th> Opis naprawy</th>
        <th> Samochód </th>
        <th> Koszt części </th>
        <th> Koszt "godzinówki" </th>
        <th> Ilość godzin naprawy </th>
        <th>Całkowity koszt naprawy</th>
    ...
    <tr>
        <td><%= order.getId() %></td>
        <td><%= order.getDate() %></td>
        <td><%= order.getPlaning_date() %></td>
        <td><%= order.getEnd_date() %></td>
        <td><%= translateStatus(order.getStatus()) %></td>
        <td><%= employeeDAO.getEmployeeById(order.getEmployee_id()).toString() %></td>
        <td><%= order.getProblem_description() %></td>
        <td><%= order.getRepair_description() %></td>
        <td><%= vehicleDAO.getVehicleById(order.getVehicle_id()).toString() %></td>
        <td><%= order.getElements_cost() %></td>
        <td><%= order.getHour_cost() %></td>
        <td><%= order.getRepair_hours() %></td>
